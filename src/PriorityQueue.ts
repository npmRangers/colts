import { SimpleSequational, Bucket } from "./Common";

/**
 * Queue which is ordered according to specified priority
 */
export class PriorityQueue<T> implements SimpleSequational<T>, Bucket<T> {
    /**
     * @note It has to be backed Heap
     */
    add(value: T): this {
    ...
    map<F>(f: (value: T, collection: this) => F): PriorityQueue<F> {
        throw new Error("Method not implemented.");
    }    
    flatMap<F>(f: (value: T, collection: this) => PriorityQueue<F>): PriorityQueue<F> {
        throw new Error("Method not implemented.");
    }
    ...
    combine(
        takeIfExistsAtThis: (value: T, collection: this) => boolean, 
        takeIfExistsAtBoth: (value: T, collection: this) => boolean, 
        takeIfExistsAtThat: (value: T, collection: this) => boolean, that: this
    ): this {
        throw new Error("Method not implemented.");
    }
